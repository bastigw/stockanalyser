...
database = MySQLDatabase('aktien-test', **{'charset': 'utf8', 'use_unicode': True, 'host': 'localhost', 'user': 'root', 'password': 'sbietk2002'})
...
class AktienJaehrlicheDaten(BaseModel):
    ...
    aktie = ForeignKeyField(column_name='aktie_id', field='aktie_id', model=AktienInformation)
...
def row_get(aktien_id: int, year: int):
    row = AktienJaehrlicheDaten.get_or_none((AktienJaehrlicheDaten.aktie_id == aktien_id) & (AktienJaehrlicheDaten.jahr == year))
    return row


def row_update(aktien_id: int, year: int, colum_value: dict) -> int:
    rows = AktienJaehrlicheDaten.update(colum_value).where((AktienJaehrlicheDaten.aktie_id == aktien_id) & (AktienJaehrlicheDaten.jahr == year)).execute()
    return rows
