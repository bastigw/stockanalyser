...
class FinanzenNetScraper(object):
    ...
    def _lookup_url(self):
       # url = "http://www.finanzen.net/mmsuggest/smartsugg.asp?max_results=1&Keywords_mode=APPROX&Keywords=%(isin)s&query=%(isin)s&bias=100&target_id=0&mmFormat=json" % {'isin': self.isin}
        url = "https://www.finanzen.net/suchergebnis.asp?_search=%(isin)s" % {'isin': self.isin}
        request = requests.get(url)
        assert "https://www.finanzen.net/aktien/" in request.url, "ISIN ist nicht vollstänidg oder falsch."
        # if "https://www.finanzen.net/aktien/" in request.url:
        return request.url
    ...
    def fetch_recent_quarterly_figures_release_date(self):
        ...
        if self.isin and "https://www.finanzen.net/aktien/" not in self.url:
